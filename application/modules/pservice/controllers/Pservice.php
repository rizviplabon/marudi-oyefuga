    ...
    function getPserviceList() {
        ...
        foreach ($data['pservices'] as $pservice) {
            ...
            if ($this->ion_auth->in_group(array('admin')) || $permis == 'ok') {
                ...
                $option1 = '<button type="button" class="btn btn-soft-info btn-xs btn_width editbutton" data-toggle="modal" data-id="' . $pservice->id . '"><i class="fa fa-edit"> ' . lang('edit') . '</i></button>';
            }

            if ($this->ion_auth->in_group(array('admin')) || $permis_2 == 'ok') {
                $option2 = '<a class="btn btn-soft-danger btn-xs btn_width delete_button" href="pservice/delete?id=' . $pservice->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i></a>';
            }
            if($this->ion_auth->in_group('admin')){
                if($this->settings->dashboard_theme =='main'){
                   ...
                   $all_options.= '
                      <a class="dropdown-item editbutton" data-toggle="modal" data-id="'. $pservice->id. '">'. lang('edit'). '</a>
                      <a class="dropdown-item" href="pservice/delete?id='. $pservice->id. '" onclick="return confirm(\'Are you sure you want to delete this item?\');">'. lang('delete'). '</a>';
            ...
            if ($pservice->active == "1") {
                $active = '<span class="badge bg-primary">'.lang('yes').'</span>';
            } else {
                $active ='<span class="badge bg-warning">'.lang('no').'</span>';
            }
