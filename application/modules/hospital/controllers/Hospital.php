...
class Hospital extends MX_Controller
{
    ...
    public function index()
    {
        ...
        foreach ($data['hospitals'] as $hospital) {
            $hospital->settings = $this->settings_model->getSettingsByHId($hospital->id);
        }
    ...
    public function addNewView()
    {
        ...
        $data['cities'] = $this->hospital_model->getCity();
        $data['countries'] = $this->hospital_model->getCountryList();
        $data['provinces'] = $this->hospital_model->getProvince();
        $this->load->view('home/dashboard');
    ...
    public function addNewHospital()
    {
        ...
        if (empty($id)) {
            ...
            $new_hospital_id = $this->input->post('new_hospital_id');
        }
    ...
    public function addNew($data1)
    {
        ...
        $theme = $this->input->post('theme');
        $new_hospital_id = $data1['new_hospital_id'];
...
For Any Support Please Contact Us At: {phone}';
    ...
    function editHospital()
    {
        ...
        $data['cities'] = $this->hospital_model->getCityList($data['hospital']->province);
        $data['countries'] = $this->hospital_model->getCountryList();
        $data['provinces'] = $this->hospital_model->getProvinceList($data['hospital']->country);
        $this->load->view('home/dashboard');
        $this->load->view('add_new', $data);
        $this->load->view('home/footer');
    }
    ...
    function checkIfUsernameAvailable()
    {
        ...
        if (!empty($id)) {
            // If id exists, exclude current hospital's username from check
            ...
            // If no id, check against all usernames
            foreach ($hospitals as $hospital) {
                $all_username[] = $hospital->username;
            }
        }
