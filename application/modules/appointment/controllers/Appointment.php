...
class Appointment extends MX_Controller
{
    ...
    public function request()
    {
        ...
        if($this->ion_auth->in_group('admin')){                
            if($this->settings->dashboard_theme == 'main'){
                $this->load->view('home/layout/header'); 
                $this->load->view('appointment/appointment_request', $data);
                $this->load->view('home/layout/footer'); 
            }else{
                $this->load->view('home/dashboard', $data);
                $this->load->view('appointment/appointment_request', $data);
    ...
    public function todays()
    {
        ...
        if($this->ion_auth->in_group('admin')){                
            if($this->settings->dashboard_theme == 'main'){
                $this->load->view('home/layout/header'); 
                $this->load->view('appointment/todays', $data);
                $this->load->view('home/layout/footer'); 
            }else{
                $this->load->view('home/dashboard', $data);
                $this->load->view('appointment/todays', $data);
    ...
    public function upcoming()
    {
        ...
        if($this->ion_auth->in_group('admin')){                
            if($this->settings->dashboard_theme == 'main'){
                $this->load->view('home/layout/header'); 
                $this->load->view('appointment/upcoming', $data);
                $this->load->view('home/layout/footer'); 
            }else{
                $this->load->view('home/dashboard', $data);
                $this->load->view('appointment/upcoming', $data);
    ...
    public function myTodays()
    {
        ...
        if ($this->ion_auth->in_group(array('Patient'))) {
            ...
            if ($user_theme == 'main') {
                ...
                $this->load->view('appointment/my_todays', $data);
    ...
    public function appointmentPayment($deposit_type, $data, $patient, $doctor, $consultant_fee, $date, $inserted_id, $redirectlink)
    {
        ...
        if ($deposit_type == 'Card') {
            ...
            if ($deposit_type == 'Account Balance') {
                ...
                if ($data['grand_total'] > $total_balance) {
                    ...
                    if ($patient_negative_balance_status == 'Yes') {
                        ...
                        $this->account_model->deductBalanceFromMultipleRows($patient, $data['grand_total']);
                    } else {
            ...
            $data_payment = array('amount_received' => $data['grand_total'], 'deposit_type' => $deposit_type, 'status' => 'paid');
            $this->finance_model->updatePayment($inserted_id, $data_payment);
            $appointment_id = $this->finance_model->getPaymentById($inserted_id)->appointment_id;
            $data_appointment_status = array('payment_status' => 'paid');
            $this->appointment_model->updateAppointment($appointment_id, $data_appointment_status);
    ...
    function getAppointmentByDoctorId()
    {
        ...
        if($this->ion_auth->in_group('admin')){                
            if($this->settings->dashboard_theme == 'main'){
                $this->load->view('home/layout/header'); 
                $this->load->view('appointment/appointment_by_doctor', $data);
                $this->load->view('home/layout/footer'); 
            }else{
                $this->load->view('home/dashboard', $data);
                $this->load->view('appointment/appointment_by_doctor', $data);
    ...
    function treatmentReport()
    {
        ...
        if($this->ion_auth->in_group('admin')){                
            if($this->settings->dashboard_theme == 'main'){
                $this->load->view('home/layout/header'); 
                $this->load->view('appointment/treatment_history', $data);
                $this->load->view('home/layout/footer'); 
            }else{
                $this->load->view('home/dashboard', $data);
                $this->load->view('appointment/treatment_history', $data);
    ...
    function myAppointments()
    {
        ...
        $this->load->view('home/dashboard', $data);
        $this->load->view('appointment/myappointments', $data);
        $this->load->view('home/footer');
    }
    ...
    function getAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($payment_details->gross_total == $total_due) {
                $bill_status = '<span class="badge badge-soft-warning">' . lang('unpaid') . '</span>';
            } elseif ($total_due == 0) {
                $bill_status = '<span class="badge badge-soft-primary">' . lang('paid') . '</span>';
            } else {
                $bill_status = '<span class="badge badge-soft-warning">' . lang('due') . '</span>';
            }
            ...
            if ($this->ion_auth->in_group(array('Doctor'))) {
                ...
                if ($appointment->status == 'Confirmed') {
                    ...
                    if ($settings->video_type == 'Zoom') {
                        if (empty($appointment->meeting_id)) {
                            $options7 = '<a class="btn btn-info btn-xs btn_width detailsbutton buttoncolor" title="' . lang('create_meeting') . '" href="meeting/zoomLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('create_meeting') . '</a>';
                        } else {
                            $options7 = '<a class="btn btn-success btn-xs btn_width buttoncolor" title="' . lang('start_live') . '" href="' . $appointment->start_url . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? \');"><i class="fa fa-headphones"></i> ' . lang('start_live') . '</a>';
                        }
                    } else {
            ...
            if($this->ion_auth->in_group('admin')){
                if($this->settings->dashboard_theme =='main'){
                   ...
                   $all_options.= '
                      <a class="dropdown-item editbutton" data-toggle="modal" data-id="'. $appointment->id. '">'. lang('edit'). '</a>
                      <a class="dropdown-item" href="appointment/delete?id='. $appointment->id. '" onclick="return confirm(\'Are you sure you want to delete this item?\');">'. lang('delete'). '</a>';
              
               $all_options.= '</div>
                  </div>';
            }else{
                $all_options = $option1 . ' ' . $option2 . ' ' . $options7;
            }}else{
                $all_options = $option1 . ' ' . $option2 . ' ' . $options7;
            }
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($appointment->date > strtotime(date('Y-m-d'))) {
                ...
                if ($this->ion_auth->in_group(array('Doctor'))) {
                    ...
                    if ($appointment->status == 'Confirmed') {
                        $settings = $this->meeting_model->getMeetingSettingsById($doctor_ion_id);
                        if ($settings && $settings->video_type == 'Zoom') {
                            if (empty($appointment->meeting_id)) {
                                $options7 = '<a class="btn btn-info btn-xs btn_width detailsbutton buttoncolor" title="' . lang('create_meeting') . '" href="meeting/zoomLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('create_meeting') . '</a>';
                            } else {
                                $options7 = '<a class="btn btn-success btn-xs btn_width buttoncolor" title="' . lang('start_live') . '" href="' . $appointment->start_url . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? \');"><i class="fa fa-headphones"></i> ' . lang('start_live') . '</a>';
    ...
    function getMyTodaysAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($patient_id == $appointment->patient) {
                ...
                if ($this->ion_auth->in_group(array('Patient'))) {
                    if ($appointment->status == 'Confirmed') {
                        // $doctor_ion_id = $this->doctor_model->getDoctorById($appointment->doctor)->ion_user_id;
                        // $settings = $this->meeting_model->getMeetingSettingsById($doctor_ion_id);
                        $settings = $this->settings_model->getSettings();
                        if ($settings->video_type == 'Zoom') {
                            $options7 = '<a class="btn btn-info btn-xs btn_width detailsbutton buttoncolor" title="' . lang('start_live') . '"  href="' . $appointment->join_url . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this Doctor? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('live') . '</a>';
                        } else {
                            $options7 = '<a class="btn btn-info btn-xs btn_width detailsbutton buttoncolor" title="' . lang('start_live') . '"  href="meeting/instantLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this Doctor? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('live') . '</a>';
                        }
                    } else {
                        $options7 = '';
                    }
    ...
    public function encrypt_appointment_data() {
        ...
        $encrypted_fields = $this->appointment_model->encrypted_fields;
        $total_count = count($appointments);
        $success_count = 0;
        $encrypted_fields_count = array_fill_keys($encrypted_fields, 0);
        
        // Re-encrypt all appointment data with our approach
        foreach ($appointments as $appointment) {
            ...
            foreach ($encrypted_fields as $field) {
                if (!empty($appointment->$field)) {
                    ...
                    $value = db_decrypt($appointment->$field);
        ...
        // Output the results
        echo "<h2>Appointment Data Encryption Results</h2>";
        echo "<p>Successfully updated {$success_count} out of {$total_count} appointments.</p>";
