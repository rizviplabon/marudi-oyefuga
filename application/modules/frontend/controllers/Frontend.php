...
class Frontend extends MX_Controller
{
    ...
    public function test()
    {
        ...
        $data['gateway'] = $this->db->get_where('paymentGateway', array('name' => $data['settings1']->payment_gateway, 'hospital_id' => 'superadmin'))->row();
            ...
            $this->load->view('frontend/front_end', $data);
    ...
    public function index()
    {
        ...
        $data['contact_message'] = $this->input->get('contact_message');
        $data['hospital_message'] = $this->input->get('hospital_message');
        $data['subscribe_message'] = $this->input->get('subscribe_message');
        ...
        $data['theme'] = $this->db->get_where('settings', array('hospital_id' => 'superadmin'))->row()->theme;
        $theme = $data['theme'];

        if ($theme == 'default') {
            $this->load->view('frontend/frontend_new', $data);
        } else {
            if (!empty($theme) && file_exists(APPPATH . 'modules/frontend/views/themes/' . $theme . '.php')) {
                $this->load->view('frontend/themes/' . $theme, $data);
            } else {
    ...
    public function update()
    {
        ...
        $after_image = $this->input->post('after_image');
        $before_image = $this->input->post('before_image');
        ...
        $review_text_under_title = $this->input->post('review_text_under_title');
        $pricing_text_under_title = $this->input->post('pricing_text_under_title');
        $news_text_under_title = $this->input->post('news_text_under_title');
        $subscribe_text_under_title = $this->input->post('subscribe_text_under_title');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->upload->do_upload('img_url')) {
                    ...
                    $data = array(
                        ...
                        'review_text_under_title' => $review_text_under_title,
                        'pricing_text_under_title' => $pricing_text_under_title,
                        'news_text_under_title' => $news_text_under_title,
                        'subscribe_text_under_title' => $subscribe_text_under_title,
                    );
                } else {
                    $data = array();
                    $data = array(
                ...
                $data_before = array();
                $file_name = $_FILES['before_image']['name'];
                $file_name_pieces = explode('_', $file_name);
                $new_file_name = '';
                $count = 1;
                foreach ($file_name_pieces as $piece) {
                ...
                if ($this->upload->do_upload('before_image')) {
                    ...
                    $data_before = array(
                        'before_image' => $img_url
                    );
                    $this->frontend_model->updateSettings($id, $data_before);
                }


                $data_after = array();
                $file_name = $_FILES['after_image']['name'];
                $file_name_pieces = explode('_', $file_name);
                $new_file_name = '';
                $count = 1;
                foreach ($file_name_pieces as $piece) {
                ...
                if ($this->upload->do_upload('after_image')) {
                    ...
                    $this->frontend_model->updateSettings($id, $data_after);
                }
    ...
    function generateHospitalId($name, $country) {
        // Step 1: Extract initials from the hospital name
        $nameParts = explode(' ', $name); // Split the hospital name into parts
        $initials = '';
        foreach ($nameParts as $part) {
            $initials .= strtoupper(substr($part, 0, 1)); // Get the first character of each part
        }

        // Step 2: Extract the first 3 characters of the country name
        $countryCode = strtoupper(substr($country, 0, 3)); // Get the first 3 characters of the country

        // Step 3: Generate a unique 3-character string
        $characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; // Allowed characters
        $uniqueString = '';
        for ($i = 0; $i < 3; $i++) {
            $uniqueString .= $characters[rand(0, strlen($characters) - 1)]; // Randomly pick characters
        }

        // Step 4: Combine all parts into the final ID
        $newHospitalId = $initials . '-' . $countryCode . '-' . $uniqueString;

        // Check if the ID is unique
        $this->db->where('new_hospital_id', $newHospitalId);
        $count = $this->db->count_all_results('hospital');
        return $count == 0 ? $newHospitalId : false; // Return the ID if unique, otherwise false
    }

    function generateUniqueHospitalId($name, $country) {
        $maxAttempts = 1000; // Maximum attempts to generate a unique ID
        $attempts = 0;

        do {
            $newHospitalId = $this->generateHospitalId($name, $country); // Generate a new ID
            $attempts++;

            if ($attempts > $maxAttempts) {
                throw new Exception("Unable to generate a unique hospital ID after $maxAttempts attempts.");
            }
        } while ($newHospitalId === false);
    ...
    public function addNewHospitalPayment()
    {
        ...
        $secretKey = $this->settings_model->getGoogleReCaptchaSettings()->secret_key;
        $token = $this->input->post('recaptcha_response');
        $ip = $_SERVER["REMOTE_ADDR"];
        $url = "https://www.google.com/recaptcha/api/siteverify?secret=" . $secretKey . "&response=" . $token . "&remoteip=" . $ip;
        $request = file_get_contents($url);
        $response = json_decode($request);
        // print_r($response);die();
         ...
         if ($response->success) {
            if ($response->score > 0.8) {
                ...
                redirect('frontend?hospital_message=failed#register_hospital');
            }
        } else {
            $this->session->set_flashdata('feedback', 'Hospital Not Created');
                redirect('frontend?hospital_message=failed#register_hospital');
        ...
        if ($trial_version == '1') {
            ...
            $data = array(
                ...
                'new_hospital_id' => $this->generateUniqueHospitalId($name, $country),
            );
...
For Any Support Please Contact with Phone No: {phone}';
    ...
    public function subscribe() {
        $data = array();
        $data['settings'] = $this->frontend_model->getSettings();
        $data['subscribes'] = $this->frontend_model->getSubscribe();
        ...
        // print_r($response);die();
         ...
         if ($response->success) {
            if ($response->score > 0.8) {
                ...
                redirect('frontend?subscribe_message=email_sent_successfullt#subscribe');
            }
        } else {
            $this->session->set_flashdata('feedback', 'Not Sent');
                redirect('frontend?subscribe_message=email_sent_successfullt#subscribe');
                ...
                $this->frontend_model->insertSubscribe($data);
                $this->session->set_flashdata('feedback', 'yes');
    ...
    function deleteSubscribe() {
        $id = $this->input->get('id');
        $this->frontend_model->deleteSubscribe($id);
        $this->session->set_flashdata('feedback', lang('deleted'));
        redirect('frontend/subscribe');
    }
