    ...
    if ($CI->router->fetch_class() == 'site' && $CI->router->fetch_method() == 'index') {
        ...
        if ($RTR->class != "cronjobs" && $RTR->class != "frontend" && $RTR->class != "payu" && $RTR->class != "status"  && $RTR->class != "request" && $RTR->class != "auth" && $RTR->class != "api") {
            if (!$CI->ion_auth->in_group(array('superadmin'))) {
                ...
                if ($CI->ion_auth->in_group(array('Patient', 'Doctor'))) {
                    $CI->language = $CI->db->get_where($group_name, array('ion_user_id' => $current_user_id))->row()->language;
        ...
        if ($RTR->class != "cronjobs" && $RTR->class != "payu" && $RTR->class != "status"  && $CI->ion_auth->in_group(array('admin', 'superadmin')) && $RTR->class != "auth" && $RTR->class != "site") {
            if ($CI->ion_auth->in_group(array('admin')) && $RTR->class != "frontend") {
                $CI->db->where('hospital_id', $CI->hospital_id);
        ...
        $common = array('storage', 'payu', 'status', 'macro', 'auth', 'pservice', 'frontend', 'settings', 'import', 'home', 'profile', 'request', 'api', 'cronjobs', 'logs', 'doctorvisit', 'site', 'category', 'roadmap', 'board','feedback', 'faq', 'blog', 'gallery', 'featured','gridsection', 'review','crutches', 'country', 'province', 'city','patientchat', 'drive', 'inventory', 'labworkflow', 'account' ); 
        ...
        if (!in_array($RTR->class, $common)) {
            if (!$CI->ion_auth->in_group(array('superadmin'))) {
                if ($RTR->class != "schedule" && $RTR->class != "meeting" && $RTR->class != "featured" && $RTR->class != "gallery" && $RTR->class != "review" && $RTR->class != "gridsection" && $RTR->class != "service" && $RTR->class != "slide") {
                    if ($RTR->class != "pgateway") {
                        // Allow access to inventory and labworkflow modules for appropriate user groups
                        if ($RTR->class == "inventory") {
                            if (!$CI->ion_auth->in_group(array('admin', 'Doctor', 'Pharmacist', 'Laboratorist'))) {
                                redirect('home/permission');
                            }
                        } elseif ($RTR->class == "labworkflow") {
                            if (!$CI->ion_auth->in_group(array('admin', 'Doctor', 'Laboratorist'))) {
                                redirect('home/permission');
                            }
                        } elseif (!in_array($RTR->class, $CI->modules)) {
                            redirect('home');
                        }
            ...
            // Handle permission checks for inventory and labworkflow modules even if they're in common array
            if ($RTR->class == "inventory" && !$CI->ion_auth->in_group(array('superadmin'))) {
                if (!$CI->ion_auth->in_group(array('admin', 'Doctor', 'Pharmacist', 'Laboratorist'))) {
                    redirect('home/permission');
                }
            } elseif ($RTR->class == "labworkflow" && !$CI->ion_auth->in_group(array('superadmin'))) {
                if (!$CI->ion_auth->in_group(array('admin', 'Doctor', 'Laboratorist'))) {
                    redirect('home/permission');
                }
            }
