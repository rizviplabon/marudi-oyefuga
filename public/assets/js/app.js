...
(function () {
  ...
  function initLeftMenuCollapse() {
    ...
    window.onload = function () {
      if (window.innerWidth >= 900 && window.innerWidth <= 1500) {
        document.body.setAttribute("sidebar-size-default", "lg");
  ...
  function initActiveMenu() {
    ...
    menuItems &&
      menuItems.forEach(function (item) {
        ...
        // Check if the href attribute is contained in the current URL
        // This makes matching more reliable for nested paths
        if (pageUrl.indexOf(item.getAttribute('href')) !== -1) {
  ...
  // Simple flag to track if we've already scrolled on this page
  window.sidebarScrolled = false;
  
  function initMenuItemScroll() {
    // Only proceed if we haven't scrolled yet on this page
    if (window.sidebarScrolled) return;
    
    // Set the flag to prevent future scrolling
    window.sidebarScrolled = true;
    ...
    setTimeout(function() {
      ...
      if (!verticalMenu || !verticalMenu.querySelector(".simplebar-content-wrapper")) return;
      
      // Get the vertical menu's height
      var menuHeight = verticalMenu.clientHeight;
      
      // Get the active menu item's position
      var menuItemOffset = activeMenu.offsetTop;
      
      // Calculate appropriate scroll position to show the active menu
      var scrollPosition = menuItemOffset - (menuHeight / 2);
      
      // Ensure scroll position is never negative
      scrollPosition = Math.max(0, scrollPosition);
      
      // Apply the scroll
      var contentWrapper = verticalMenu.querySelector(".simplebar-content-wrapper");
      if (contentWrapper) {
        contentWrapper.scrollTop = scrollPosition;
      }
    }, 500); // Use a longer timeout to ensure DOM is fully ready
  }

  // Call initActiveMenu whenever page loads
  document.addEventListener('DOMContentLoaded', function() {
    // Reset the flag on new page load
    window.sidebarScrolled = false;
    
    // Set active menu items
    initActiveMenu();
    
    // Scroll to active menu with a slight delay
    initMenuItemScroll();
  });

  // Handle AJAX navigations - only update active menu, don't scroll
  $(document).ajaxComplete(function() {
    initActiveMenu();
  });

  // Reset scroll flag when navigating to a new page
  ...
  // For link clicks that might navigate
